...
-- 고객 이름 두번재 글자가 '지'인 고객 검색
select * from customer where custname like '_지%';

-- 고객 이름 세번재 글자가 '수'인 고객
...
select * from customer where right(phone, 1) != '4';
...
-- 주의) order by 절은 where 절 보다 뒤에 위치해야 한다.
...
select sum(amount) as '총 판매량' from orders ;
...
-- 9. group by : "~별로"
-- 고객별로 주문한 주문 건수 구하기
-- select 절에서 group by 에서 사용한 속성과 집계함수만 사용 가능하다.
...
having 
- 그룹에 대해서 필터링 (그래서 group by 와 함께 쓰인다.)
- group by 보다 뒤에 위치해야 한다
- 집계 함수랑 함께 사용 가능하다.
