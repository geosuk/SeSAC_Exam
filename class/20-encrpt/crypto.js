...
console.log(createHashedPw('1233')); // 미세한 변화에도 hash 값은 완전히 다르다.
...
// 단방향 암호화 생성 함수
// randomBytes(16) 16바이트 사이즈의 랜덤 값
...
const saltAndHashPw = (pw) => {
    ...
    const hash = crypto.
        ...
        toString('base64'); // 암호화된 Buffer 형식의 데이터를 "base64 문자열로 변환"해서 저장하거나 전송하기 수비도록
...
const comparePw = (inputPw, savedSalt, savedHash) => {
    // saltAndHashPw 함수에서 정의한 값들이랑 일치해야 한다.
    const iterations = 100000; // 반복 횟수
    const keylen = 64; // 키의 길이
    const digest = 'sha512'; // 해시 알고리즘
